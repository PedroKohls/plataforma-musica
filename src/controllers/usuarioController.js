...
module.exports = {
  ...
  cadastrar: async function (req, res) {
    const pastaUsuarios = path.join(__dirname, "../public/imagens/usuarios");

    await fs.promises.mkdir(pastaUsuarios, { recursive: true });
  ...
  Inicio: async function (req, res) {
    const usuarioId =
      req.session && req.session.usuario ? req.session.usuario.id : null;
        ...
        include: [
          {
            model: musicaModel,
            as: "Musica",
            include: [{ model: artistaModel, as: "Artista" }],
          },
        ],
      }),
